 ...
 */
public class NeuralRobot extends RobotWithMultipleSensors {

    protected double[][] sensoryData;

    public final int NUM_SENSORS = 10;

    public NeuralRobot(float x, float y) {
        super(x, y);
        sensors = new Vector<Sensor>();
        sensorsMap = new HashMap<Integer, Sensor>();
    }

    public NeuralRobot(Shape shape, int layer, Arena arena) {
    ...
    public float getAcceleration() {
        return vivae.arena.parts.Robot.ACCELERATION;
    }

    @Override
    public float getMaxSpeed() {
        return vivae.arena.parts.Robot.MAX_SPEED;
    }

    @Override
    public float getRotationIncrement() {
        return vivae.arena.parts.Robot.ROTATION;
    }
